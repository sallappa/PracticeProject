...
public class Main2 {
	...
	private static void optionsSelection() {
		...
		if (options < 1 || options > 6) {
			...
			optionsSelection(); // Restart the selection process
	...
	private static void searchExpenses(ArrayList<Integer> arrayList) {
		...
		for (int i = 0; i < arrayList.size(); i++) {
			int expense = arrayList.get(i);
			if (expense == searchValue) {
				found = true;
				System.out.println("Expense found at index " + i + ".\n");
				break; // Exit loop as soon as expense is found
			}
		}
	...
	private static void sortExpenses(ArrayList<Integer> arrayList) {
		Collections.sort(arrayList); // Use the built-in sort function
